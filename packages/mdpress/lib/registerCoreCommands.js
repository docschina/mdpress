...
const { chalk } = require('@mdpress/shared-utils');
const envinfo = require('envinfo');

const { dev, build, eject } = require('@mdpress/core');
const { path, logger, env } = require('@mdpress/shared-utils');
...
module.exports = function (cli, options) {
  ...
  cli
    ...
    .option('-d, --dest <dest>', 'specify build output dir (default: .mdpress/dist)')
    .option('-t, --temp <temp>', 'set the directory of the temporary file')
    .option('-c, --cache [cache]', 'set the directory of cache')
    ...
    .option('--keep-manifest', 'keep manifest files')
    .action((sourceDir = '.', commandOptions) => {
      const { debug, silent } = commandOptions;
  ...
  cli
    .command('eject [targetDir]', 'copy the default theme into .mdpress/theme for customization.')
