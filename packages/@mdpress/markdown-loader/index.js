...
const { fs, path, hash, parseFrontmatter, inferTitle, extractHeaders } = require('@mdpress/shared-utils');
const LRU = require('lru-cache');
const md = require('@mdpress/markdown');
...
module.exports = function (src) {
  ...
  // babel-loader, and will be applied on the same file multiple times when
  // selecting the individual blocks.
  const file = this.resourcePath;
  const key = hash(file + src);
  const cached = cache.get(key);
  if (cached && (isProd || /\?js/.test(this.resourceQuery))) {
    return cached;
  }
  ...
  const res = (
    ...
    export default function R(props) {
        const { className = '',slotKey } = props;
        return (
            <React.Fragment>
               <ContentSlotsDistributor slotKey={slotKey} className={className} markDownString={str}/>
            </React.Fragment>
       )
    }
    `
