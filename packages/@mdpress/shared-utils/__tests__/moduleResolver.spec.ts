jest.mock('mdpress-plugin-a');
jest.mock('@org/mdpress-plugin-a');
jest.mock('@mdpress/plugin-a');

jest.mock('mdpress-theme-a');
jest.mock('@org/mdpress-theme-a');
jest.mock('@mdpress/theme-a');
...
describe('resolveScopePackage', () => {
  test('corrent format', () => {
    const pkg = resolveScopePackage('@mdpress/plugin-a');
    expect(pkg.org).toBe('mdpress');
    expect(pkg.name).toBe('plugin-a');
  });

  test('incorrect format', () => {
    const pkg2 = resolveScopePackage('mdpress/plugin-a');
    expect(pkg2).toEqual({ 'name': '', 'org': '' });

    const pkg3 = resolveScopePackage('mdpress-plugin-a');
    expect(pkg3).toEqual({ 'name': '', 'org': '' });
  });
});

const getBaseAsserts = (type: string) => [
  { input: 'a', output: ['a', `mdpress-${type}-a`] },
  { input: `mdpress-${type}-a`, output: ['a', `mdpress-${type}-a`] },
  { input: '@mdpress/a', output: ['@mdpress/a', `@mdpress/${type}-a`] },
  { input: '@org/a', output: ['@org/a', `@org/mdpress-${type}-a`] }
];
...
describe('normalizeRequest', () => {
  ...
  test('object', () => {
    ...
    expect(name).toBe(`mdpress-plugin-anonymous-${hash(req)}`);
  });
  ...
  test('object - should respect name', () => {
    ...
    expect(name).toBe('mdpress-plugin-a');
  });
});
