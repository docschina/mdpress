const { fs, path, logger, chalk } = require('@mdpress/shared-utils');
...
module.exports = (options, ctx) => ({
  ...
  async ready () {
    ...
    const overridePath = path.resolve(sourceDir, '.mdpress/override.styl');
    const hasUserOverride = fs.existsSync(overridePath);

    if (hasUserOverride) {
      logger.tip(`${chalk.magenta('override.styl')} has been deprecated from v1.0.0, using ${chalk.cyan('.mdpress/styles/palette.styl')} instead.\n`);
    }

    const themeStyle = path.resolve(themeAPI.theme.path, 'styles/index.styl');
    const userStyle = path.resolve(sourceDir, '.mdpress/styles/index.styl');
