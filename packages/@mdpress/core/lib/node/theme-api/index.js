...
module.exports = class ThemeAPI {
  constructor (theme, parentTheme) {
    ...
    this.mdpressPlugin = {
      name: '@mdpress/internal-theme-api',
      alias: {}
    };
    this.init();
  }
  ...
  getLayoutComponentMap () {
    ...
    if (!Layout) {
      const fallbackLayoutPath = resolve(__dirname, 'Layout.fallback.js');
      ...
      logger.warn(
        `[mdpress] Cannot resolve Layout.js file in \n ${layoutDirs.join('\n')}, `
    ...
    if (!NotFound) {
      layoutComponentMap.NotFound = {
        ...
        path: resolve(__dirname, '../../client/components/NotFound.js'),
        isInternal: true
      };
    }
...
/**
 * Resolve React SFCs, return a Map
 *
 * @param dirs
 * @returns {*}
 */
