...
const { frontmatterEmitter } = require('@mdpress/markdown-loader');
const webpack = require('webpack');
const chokidar = require('chokidar');

const { path, fs, logger } = require('@mdpress/shared-utils');
...
module.exports = class DevProcess extends EventEmitter {
  ...
  watchSourceFiles () {
    ...
    this.pagesWatcher = chokidar.watch([
      '**/*.md',
      '.mdpress/components/**/*.js',
      '.mdpress/components/**/*.jsx',
    ], {
      cwd: this.context.sourceDir,
      ignored: ['.mdpress/**/*.md', 'node_modules'],
      ignoreInitial: true
    });
  ...
  createServer () {
    const contentBase = path.resolve(this.context.sourceDir, '.mdpress/public');
