...
module.exports = class Build extends EventEmitter {
  constructor (context,removeMainfest) {
    super();
    this.context = context;
    this.outDir = this.context.outDir;
    this.removeMainfest = removeMainfest;
  }
  ...
  async render () {
    ...
    if (this.removeMainfest) await fs.remove(path.resolve(this.outDir, 'manifest'));
    ...
    logger.developer(`It took a total of ${chalk.cyan(`${duration}ms`)} to run the ${chalk.cyan('mdpress build')}.`);
    console.log();
  }
