 ...
 *   2. If 'theme' directory located at mdpressDir exists, respect it as
 *      the theme directory.
...
module.exports = function loadTheme (ctx) {
  ...
  if (!theme.path) {
    throw new Error(
      '[mdpress] You must specify a theme, or create a local custom theme. \n'
        + 'For more details, refer to docs custom-themes. \n'
    );
  }

  let applyTip = `Apply theme ${chalk.magenta(theme.name)}`;
  theme.entry.name = '@mdpress/internal-theme-entry-file';

  let parentTheme = {};
  if (theme.entry.extend) {
    parentTheme = resolveTheme(ctx, themeResolver, true, theme.entry.extend);
    parentTheme.entry.name = '@mdpress/internal-parent-theme-entry-file';
...
function resolveTheme (ctx, resolver, ignoreLocal, theme) {
  const { siteConfig, options, sourceDir, userDir, pluginAPI } = ctx;
  const localThemePath = resolve(userDir, 'theme');
  ...
  /**
   * 1. From `.mdpress/theme` directory.
   */
  if (
    !ignoreLocal
    && !fs.existsSync(theme)
