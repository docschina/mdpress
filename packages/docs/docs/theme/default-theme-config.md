...
MdPress checks each sidebar config from top to bottom. If the fallback configuration was first, MdPress would incorrectly match `/foo/` or `/bar/four.html` because they both start with `/`.
:::
...
Then you can write CSS targeting that page only in `./mdpress/styles/index.styl`.
...
This will render `.mdpress/components/SpecialLayout.js` for the given page.

## Ejecting

You can copy the default theme source code into `.mdpress/theme` to fully customize the theme using the `mdpress eject [targetDir]` command.

::: warning
Once you eject, you are on your own and **won’t** be receiving future updates or bugfixes to the default theme even if you upgrade MdPress.
:::
